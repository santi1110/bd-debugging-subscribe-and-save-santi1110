...
public class SubscriptionService {
    ...
    public Subscription subscribe(String customerId, String asin, int frequency) {
        if (StringUtils.isBlank(customerId) || StringUtils.isBlank(asin)) {
            throw new IllegalArgumentException(String.format("Invalid inputs. A Customer ID and ASIN must be " +
                                                                 "provided. Provided: {Customer ID: %s, ASIN: %s}",
                                                             customerId, asin));
        }

        if (frequency < 1 || frequency > 6) {
            throw new IllegalArgumentException(
                String.format("Invalid frequency value. Please provide how often (in months) the " +
                                  "subscription should occur - between 1 and 6. Provided: {Frequency: %d}", frequency));
        }

        if (!identityService.validateCustomer(customerId)) {
            throw new IllegalArgumentException(String.format("Unable to create subscription for customerId: %s. " +
